...
// and similarly for the third set of fifteen bits and then for the remaning last four bits fianlly we add them all to
// get a 64 bit no.
#define rand_64 ((ULL)rand() | (ULL)rand() << 15 | (ULL)rand() << 30 | ((ULL)rand() << 45 | ((ULL)rand() & 0xf) << 60))
...
ULL CastleKey[16];     // To store 64  bits random numbers which represent each possible type of castlepositions which are
                       // 0 to 15.
int FileNo[SQNO];
int RankNo[SQNO];
...
void Convert()
{
    ...
    for (int rank = 0; rank < 8; rank++)
    {
        for (int file = 0; file < 8; file++)
        {
            sq120 =
                fr2sq(file,
                      rank);  // Move across ranks and files and convert the respective indexes to 120 based squares.
            sq120to64[sq120] =
                sq64;  // Then at that index give the value of square value at 64 size board and vice versa.
