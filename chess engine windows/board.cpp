...
//this function takes in an FEN notation string and sets the board accorting to that...it assigns all the pieces to 120 size board
//and then finally generates the uniques PosKey value for that arrangement...but it doesnt computes PieceList,MajPce,MinPce
...
int CheckBoard(BOARD *board)
{
	...
	//now ASSERT the maj,min,big pieces and valuepieces
	...
	ASSERT(board->EnPas==NONE ||(RankNo[board->EnPas]==RANK_6 && board->Side==WHITE) || (RankNo[board->EnPas]==RANK_3 && board->Side==BLACK));
